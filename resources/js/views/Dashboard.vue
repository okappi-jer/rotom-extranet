      ...
      <div class="quick--actions" v-if="$store.getters.getUser.role !== 'Visitor' && $store.getters.getUser.role !== 'DeliveryAdmin'">
          ...
          <div class="status-panel">
              <a href="/#/borderels">
                ...
                <small>Mijn aankoopborderels</small>
              </a>
          </div>
          <div class="status-panel">
              <a href="/#/history">
                <span class="big">
                  ...
                  <small>Historiek leveringen</small>
              </a>
          </div>
      </div>
...
export default {
  ...
  mounted() {
    if (this.$store.getters.getUser) {
      this.$store.dispatch('appStore/' + IS_LOADING, false, { root: true });
    } else {
      // If no user, try again after 500 milisecs
      this.$store.dispatch('appStore/' + IS_LOADING, true, { root: true });
      setTimeout(() => { this.$store.dispatch('appStore/' + IS_LOADING, false, { root: true });}, 500);
    }
  },
  computed: {
    user() {
